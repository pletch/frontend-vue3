...
- MapLibre is now loaded as its own chunk rather than in the entry bundle, and the framework and date-handling dependencies are split out from application code so that a redeploy does not invalidate them in the browser cache. The bytes needed before the interface can render dropped from 414 KB to 129 KB gzipped; the map library is fetched in parallel as the app boots.
