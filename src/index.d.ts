...
interface OTLocation {
  ...
  ghash?: string;
  /**
   * A list of regions the device is currently in, e.g. ["Home", "Garage"].
   * Might be empty.
   */
  inregions?: string[];
  /**
   * A list of region IDs the device is currently in, e.g. ["6da9cf"].
   * Might be empty.
   */
  inrids?: string[];
  /**
  ...
  name?: string;
  /**
   * Motion activities reported by the device, e.g. ["walking"]. Drives the
   * activity glyph on markers and during playback.
   */
   ...
   * - `"v"` = updated by Settings/Privacy/Location Services/System Services/
   *   Frequent Locations monitoring (iOS only)
   */
  t?: string;
